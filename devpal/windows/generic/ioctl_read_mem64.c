...
{
    NTSTATUS status;
    struct read_mem64_operands* operands_in;
    struct read_mem64_operands* operands_out;
    size_t in_buffer_size = 0;
    size_t out_buffer_size = 0;

    status = WdfRequestRetrieveInputBuffer(Request, InputBufferLength, &operands_in, &in_buffer_size);
    if (!NT_SUCCESS(status) || in_buffer_size < sizeof(struct read_mem64_operands)) {
        WdfRequestComplete(Request, STATUS_ACCESS_DENIED);
        return;
    }

    status = WdfRequestRetrieveOutputBuffer(Request, OutputBufferLength, &operands_out, &out_buffer_size);
    if (!NT_SUCCESS(status) || out_buffer_size < sizeof(struct read_mem64_operands)) {
        WdfRequestComplete(Request, STATUS_ACCESS_DENIED);
        return;
    }
    ...
    RtlCopyMemory(operands_out, operands_in, sizeof(struct read_mem64_operands));

    operands_out->out.value = *(PUINT64)(operands_in->in.address);
